...
//       
//       Creation Date:  Tue Jun 30 13:08:50 2026
//       Version: 	r0p0
//
...
`ifdef POWER_PINS
module rf64x128 (VDDCE, VDDPE, VSSE, q, clk, cen, wen, a, d, ema, emaw, emas, gwen,
    ret1n, wabl, wablm, rawl, rawlm);
`else
module rf64x128 (q, clk, cen, wen, a, d, ema, emaw, emas, gwen, ret1n, wabl, wablm,
    rawl, rawlm);
`endif
 ...
 wire  clk_;
  ...
  assign rawlm_[1] = rawlm[1];
   ...
   `ifdef ARM_FAULT_MODELING
     rf64x128_error_injection u1(.CLK(clk_), .Q_out(q_), .A(a_int), .CEN(cen_int), .GWEN(gwen_int), .WEN(wen_int), .Q_in(q_int));
  `else
...
`endif
`timescale 1ns/1ps
module rf64x128_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [127:0] Q_out;
   input [127:0] Q_in;
   input CLK;
   input [5:0] A;
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(6'd1,7'd70,2'd1,2'd0);
   `endif
end
   task add_fault;

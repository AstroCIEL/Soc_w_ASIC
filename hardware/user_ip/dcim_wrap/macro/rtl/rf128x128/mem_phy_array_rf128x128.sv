// Layer 3: physical macro wrapper (128-row macro depth variant).
// Per-macro request is driven by macro_ena; be is the bit-level write mask only.
module mem_phy_array_rf128x128 #(
	...
	initial begin
		if (DATA_WIDTH % MACRO_WIDTH != 0) begin
			$error("rf_wrap_128x128: DATA_WIDTH must be an integer multiple of MACRO_WIDTH");
		end
		if (DATA_DEPTH % MACRO_DEPTH != 0) begin
			$error("rf_wrap_128x128: DATA_DEPTH must be an integer multiple of MACRO_DEPTH");
		end
	end

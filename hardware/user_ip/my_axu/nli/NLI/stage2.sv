module stage2#(
    ...
    dff #(.WIDTH(MACRO_INDEX_WIDTH), .DEPTH(MULTIPLY_LATENCY)) u_dff_pipe2(
        .clk(clk),  .rstn(rstn),    .ena(1'b1),
        .data_in(macro_index_pipe1),
        .data_out(macro_index_pipe2)
    );

    pipe_ctrl #(.PIPE_DEPTH(MULTIPLY_LATENCY)) u_pipe2_ctrl(
        .clk(clk),  .rstn(rstn),    .clr(1'b0), .ena(1'b1),
    ...
    assign interval_index_temp = (macro_index_pipe2==0)? 0: 1 + (macro_index_pipe2-1)*NUM_MICROS + micro_index;

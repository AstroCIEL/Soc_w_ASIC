...
// ACC精度专用2级流水线加法核
module posit_acc_add_kernel_pipe2 #(
    ...
    // 输入：posit_acc_t 精度（全累加器位宽）
    input  logic                                     a_sign_i,
    input  logic signed [get_max_exp_width(n_i, es_i, n_o, es_o):0]      a_rg_exp_i,
    input  logic        [get_mant_width_o(n_o, es_o)+2:0]   a_mant_i,
    
    input  logic                                     b_sign_i,
    input  logic signed [get_max_exp_width(n_i, es_i, n_o, es_o):0]      b_rg_exp_i,
    input  logic        [get_mant_width_o(n_o, es_o)+2:0]      b_mant_i,

    //输出：posit_acc_t 精度（全累加器位宽）
    ...
    // 本地参数：全部绑定 ACC 精度位宽
    localparam int unsigned EXP_WIDTH    = get_max_exp_width(n_i, es_i, n_o, es_o);   // ACC指数位宽
    localparam int unsigned MANT_WIDTH   = get_mant_width_o(n_o, es_o)+2; // ACC尾数位宽
    ...
    // ======================================
    logic                        a_sign_r;
    logic signed [EXP_WIDTH:0]   a_rg_exp_r;
    logic        [MANT_WIDTH:0]   a_mant_r;
    logic                        b_sign_r;
    logic signed [EXP_WIDTH:0]   b_rg_exp_r;
    logic        [MANT_WIDTH:0]   b_mant_r;
    ...
    assign operand_aligned[0] = (ALIGN_WIDTH > MANT_WIDTH + 1) ? 
        (a_mant_r << (ALIGN_WIDTH - MANT_WIDTH - 1)) : 
        (a_mant_r >> (MANT_WIDTH + 1 - ALIGN_WIDTH));
    assign operand_aligned[1] = (ALIGN_WIDTH > MANT_WIDTH + 1) ? 
        (b_mant_r << (ALIGN_WIDTH - MANT_WIDTH - 1)) : 
        (b_mant_r >> (MANT_WIDTH + 1 - ALIGN_WIDTH));
    ...
    logic [MANT_WIDTH:0] final_mant;
    assign final_rg_exp = rg_exp_max + rg_exp_adjust;
    assign final_mant = (SUM_WIDTH > MANT_WIDTH+1) ? 
        sum_norm[SUM_WIDTH-1 : SUM_WIDTH - MANT_WIDTH - 1] : 
        (sum_norm << (MANT_WIDTH + 1 - SUM_WIDTH));

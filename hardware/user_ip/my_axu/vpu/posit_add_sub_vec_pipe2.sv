...
/*
 20260505：
 适配posit_add_sub_kernel_pipe2.sv, 两级流水线，输入输出为posit格式，带encoder、decoder。
*/
module posit_add_sub_vec_pipe2 #(
    ...
    output logic [n-1:0]  vec_result_posit_o[NUM_ADDER-1:0]
);
...
generate
    for (genvar i = 0; i < NUM_ADDER; i++) begin:gen_encoder
        posit_encoder #(.n(n), .es(es), .EXP_WIDTH(MAX_EXP_W), .MANT_WIDTH(MANT_WIDTH_O+2)) u_enc (
            .sign_i(sum_sign[i]), .rg_exp_i(sum_rg_exp[i]), .mant_norm_i(sum_mant[i]), .result_o(vec_result_posit_o[i])
        ); 
    end
endgenerate

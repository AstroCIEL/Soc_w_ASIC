// Posit encoder
// 0513:发现对于最大值输入7fff，regime位编码错误，因此增加对max和min的单独讨论。
module posit_encoder #(
    ...
    // 单独处理最大最小值：7fff和8000
    logic is_max_positive, is_min_negative;
    assign is_max_positive = (~sign_i) & (regime_bits >= n);
    assign is_min_negative = sign_i & (regime_bits >= n);

    assign result_o = 
        ~input_not_zero ? '0 :
        is_max_positive ? {1'b0, {(n-1){1'b1}}} :  // 最大值：0x7fff
        is_min_negative ? {1'b1, {(n-1){1'b0}}} :  // 最小值：0x8000
        normal_result;

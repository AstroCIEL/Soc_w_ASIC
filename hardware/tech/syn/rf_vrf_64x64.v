...
//       
//       Creation Date:  Sat May  9 08:44:43 2026
//       Version: 	r3p1
//
...
`ifdef POWER_PINS
module rf_vrf_64x64 (VDDCE, VDDPE, VSSE, q, clk, cen, wen, a, d, ema, emaw, emas, gwen, 
    ret1n, wabl, wablm, rawl, rawlm);
`else
module rf_vrf_64x64 (q, clk, cen, wen, a, d, ema, emaw, emas, gwen, ret1n, wabl, wablm, 
    rawl, rawlm);
`endif
 ...
 wire  clk_;
  ...
  assign rawlm_[1] = rawlm[1];
   ...
   `ifdef ARM_FAULT_MODELING
     rf_vrf_64x64_error_injection u1(.CLK(clk_), .Q_out(q_out), .A(a_int), .CEN(cen_int), .GWEN(gwen_int), .WEN(wen_int), .Q_in(q_int));
   `else
     assign q_out = q_int;
   `endif
  assign `ARM_UD_SEQ q_ = (ret1n_ | pre_charge_st) & ~corrupt_power ? ((q_out)) : {64{1'bx}};
 ...
 wire  clk_;
  ...
  buf B213(rawlm_[1], rawlm[1]);
   ...
   `else
     assign q_out = q_int;
   `endif
  assign q_ = (ret1n_ | pre_charge_st) & ~corrupt_power ? ((q_out)) : {64{1'bx}};
...
`timescale 1ns/1ps
`ifdef ARM_FAULT_MODELING
module rf_vrf_64x64_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [63:0] Q_out;
   input [63:0] Q_in;
   input CLK;
   input [5:0] A;
...
initial
begin
   `ifdef rf_vrf_64x64_pre_pend_path
      `define pre_pend_path `rf_vrf_64x64_pre_pend_path
   `else
      `ifdef DUT
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.u1.add_fault(6'd1,6'd6,2'd1,2'd0);
   `endif
end
   task add_fault;

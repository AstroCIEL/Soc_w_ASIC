...
//
//       Instance Name:              rf_icache_tag_64x48
//       Words:                      64
//       Bits:                       48
//       Mux:                        2
//       Drive:                      6
...
//       
//       Creation Date:  Sat May  9 08:46:34 2026
//       Version: 	r3p1
//
...
`ifdef POWER_PINS
module rf_icache_tag_64x48 (VDDCE, VDDPE, VSSE, q, clk, cen, wen, a, d, ema, emaw, 
    emas, gwen, ret1n, wabl, wablm, rawl, rawlm);
`else
module rf_icache_tag_64x48 (q, clk, cen, wen, a, d, ema, emaw, emas, gwen, ret1n, wabl, 
    wablm, rawl, rawlm);
`endif
  ...
  parameter WORDS = 64;
  parameter MUX = 2;
  parameter MEM_WIDTH = 96; // redun block size 2, 48 on left, 48 on right
 ...
 wire  clk_;
  ...
  assign rawlm_[1] = rawlm[1];
   ...
   `ifdef ARM_FAULT_MODELING
     rf_icache_tag_64x48_error_injection u1(.CLK(clk_), .Q_out(q_out), .A(a_int), .CEN(cen_int), .GWEN(gwen_int), .WEN(wen_int), .Q_in(q_int));
   `else
     assign q_out = q_int;
   `endif
  assign `ARM_UD_SEQ q_ = (ret1n_ | pre_charge_st) & ~corrupt_power ? ((q_out)) : {48{1'bx}};
...
`endif
      ...
      row = mem[row_address];
        writeEnable = {48{1'b1}};
        row_mask =  ( {1'b0, writeEnable[47], 1'b0, writeEnable[46], 1'b0, writeEnable[45],
        ...
        new_data =  ( {1'b0, wordtemp[47], 1'b0, wordtemp[46], 1'b0, wordtemp[45],
...
`endif
      ...
      row = mem[row_address];
        ...
        readLatch0 = {data_out[94], data_out[92], data_out[90], data_out[88], data_out[86],
        ...
        q_int = {shifted_readLatch0[47], shifted_readLatch0[46], shifted_readLatch0[45],
...
`endif
  ...
  begin
    ...
    if (ret1n_int === 1'bx || ret1n_int === 1'bz) begin
      ...
      q_int = gwen_int !== 1'b1 ? q_int : {48{1'bx}};
     ...
     if (gwen_int !== 1)
      ...
      if( isBitX(gwen_int) ) begin
        ...
        d_int = {48{1'bx}};
          ...
          writeEnable = ~ ( {48{gwen_int}} | {wen_int[47], wen_int[46], wen_int[45],
          wen_int[44], wen_int[43], wen_int[42], wen_int[41], wen_int[40], wen_int[39],
          wen_int[38], wen_int[37], wen_int[36], wen_int[35], wen_int[34], wen_int[33],
      ...
      if (gwen_int !== 1'b1) begin
        row_mask =  ( {1'b0, writeEnable[47], 1'b0, writeEnable[46], 1'b0, writeEnable[45],
          ...
          1'b0, writeEnable[4], 1'b0, writeEnable[3], 1'b0, writeEnable[2], 1'b0, writeEnable[1],
          1'b0, writeEnable[0]} << mux_address);
        new_data =  ( {1'b0, d_int[47], 1'b0, d_int[46], 1'b0, d_int[45], 1'b0, d_int[44],
...
`endif
  ...
  reg NOT_cen, NOT_wen47, NOT_wen46, NOT_wen45, NOT_wen44, NOT_wen43, NOT_wen42, NOT_wen41;
  ...
  reg NOT_wen4, NOT_wen3, NOT_wen2, NOT_wen1, NOT_wen0, NOT_a5, NOT_a4, NOT_a3, NOT_a2;
  reg NOT_a1, NOT_a0, NOT_d47, NOT_d46, NOT_d45, NOT_d44, NOT_d43, NOT_d42, NOT_d41;
 ...
 wire  clk_;
  ...
  buf B48(clk_, clk);
  buf B49(cen_, cen);
  buf B50(wen_[0], wen[0]);
  buf B51(wen_[1], wen[1]);
  buf B52(wen_[2], wen[2]);
  buf B53(wen_[3], wen[3]);
  buf B54(wen_[4], wen[4]);
  buf B55(wen_[5], wen[5]);
  buf B56(wen_[6], wen[6]);
  buf B57(wen_[7], wen[7]);
  buf B58(wen_[8], wen[8]);
  buf B59(wen_[9], wen[9]);
  buf B60(wen_[10], wen[10]);
  buf B61(wen_[11], wen[11]);
  buf B62(wen_[12], wen[12]);
  buf B63(wen_[13], wen[13]);
  buf B64(wen_[14], wen[14]);
  buf B65(wen_[15], wen[15]);
  buf B66(wen_[16], wen[16]);
  buf B67(wen_[17], wen[17]);
  buf B68(wen_[18], wen[18]);
  buf B69(wen_[19], wen[19]);
  buf B70(wen_[20], wen[20]);
  buf B71(wen_[21], wen[21]);
  buf B72(wen_[22], wen[22]);
  buf B73(wen_[23], wen[23]);
  buf B74(wen_[24], wen[24]);
  buf B75(wen_[25], wen[25]);
  buf B76(wen_[26], wen[26]);
  buf B77(wen_[27], wen[27]);
  buf B78(wen_[28], wen[28]);
  buf B79(wen_[29], wen[29]);
  buf B80(wen_[30], wen[30]);
  buf B81(wen_[31], wen[31]);
  buf B82(wen_[32], wen[32]);
  buf B83(wen_[33], wen[33]);
  buf B84(wen_[34], wen[34]);
  buf B85(wen_[35], wen[35]);
  buf B86(wen_[36], wen[36]);
  buf B87(wen_[37], wen[37]);
  buf B88(wen_[38], wen[38]);
  buf B89(wen_[39], wen[39]);
  buf B90(wen_[40], wen[40]);
  buf B91(wen_[41], wen[41]);
  buf B92(wen_[42], wen[42]);
  buf B93(wen_[43], wen[43]);
  buf B94(wen_[44], wen[44]);
  buf B95(wen_[45], wen[45]);
  buf B96(wen_[46], wen[46]);
  buf B97(wen_[47], wen[47]);
  buf B98(a_[0], a[0]);
  buf B99(a_[1], a[1]);
  buf B100(a_[2], a[2]);
  buf B101(a_[3], a[3]);
  ...
  buf B150(d_[46], d[46]);
  buf B151(d_[47], d[47]);
  buf B152(ema_[0], ema[0]);
  buf B153(ema_[1], ema[1]);
  buf B154(ema_[2], ema[2]);
  buf B155(emaw_[0], emaw[0]);
  buf B156(emaw_[1], emaw[1]);
  buf B157(emas_, emas);
  buf B158(gwen_, gwen);
  buf B159(ret1n_, ret1n);
  buf B160(wabl_, wabl);
  buf B161(wablm_[0], wablm[0]);
  buf B162(wablm_[1], wablm[1]);
  buf B163(rawl_, rawl);
  buf B164(rawlm_[0], rawlm[0]);
  buf B165(rawlm_[1], rawlm[1]);
   ...
   `else
     assign q_out = q_int;
   `endif
  assign q_ = (ret1n_ | pre_charge_st) & ~corrupt_power ? ((q_out)) : {48{1'bx}};
...
`endif
  ...
  wire rawleq1awableq0aret1neq1aceneq0, rawleq0awableq0aret1neq1aceneq0, ret1neq1aceneq0awen47eq0agweneq0;
...
`timescale 1ns/1ps
`ifdef ARM_FAULT_MODELING
module rf_icache_tag_64x48_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, GWEN);
   output [47:0] Q_out;
   input [47:0] Q_in;
   input CLK;
   input [5:0] A;
...
initial
begin
   `ifdef rf_icache_tag_64x48_pre_pend_path
      `define pre_pend_path `rf_icache_tag_64x48_pre_pend_path
   `else
      `ifdef DUT
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.u1.add_fault(6'd1,6'd22,2'd1,2'd0);
   `endif
end
   task add_fault;

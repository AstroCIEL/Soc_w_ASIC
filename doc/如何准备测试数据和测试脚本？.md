...
| `software/app/dcim_test/` | DCIM baremetal smoke / kick 测试 |
| `dcim_shell.sh` | DCIM 一键编译 + 仿真 + grep `DCIM_PASS` |

### 1.3 MXU / AXU / iDMA / DCIM 测试链路对比

| 项目 | MXU | AXU | iDMA + GlobalBuffer | DCIM |
|---|---|---|---|---|
| workspace | `zzc_workspace_mxu` | `zzc_workspace_axu` | app 内 pattern/poison | app 内 pattern（smoke 阶段） |
| golden 目录 | `gloden_result` | `golden_result` | app 内 expected | 暂未接入（smoke 无 golden） |
| 生成脚本 | `gen_input_data.py` | `gen_input_data_axu.py` | 通常不需要 | 暂未需要 |
| 构建变量 | `MXU_TEST_MODE` | `AXU_TEST_CASE` | 无专用 case 变量 | `DCIM_TEST_TOPO`、`DCIM_WAIT_CYCLES` |
| app | `my_mxu_test` | `my_axu_test` | `mxu_idma_gbuf_test` | `dcim_test` |
| 通过 marker | `MXU_PASS` | `AXU_PASS` | `MXU_IDMA_GBUF_PASS` | `DCIM_PASS` |
| 验证重点 | 计算正确性 | 多 case 功能正确性 | DMA master 地址可达性 | MMIO region / buffer / kick 流程 |

...
#(
  ...
  for (genvar i = 0; i < ICACHE_SET_ASSOC; i++) begin : gen_sram
    ...
    rf_be_128x46 i_tag_rf (
      ...
      .addr_i         ( vld_addr                 ),
      .write_data_i   ( {vld_wdata[i], cl_tag_q} ),
      .bit_enable_i   ( '1                       ),
      .read_data_o    ( cl_tag_valid_rdata[i]    )
    );
    ...
    rf_be_128x128 i_data_rf (
      .clk_i          ( clk_i           ),
      .chip_enable_i  ( cl_req[i]       ),
      .write_enable_i ( cl_we           ),
      .addr_i         ( cl_index        ),
      .write_data_i   ( mem_rtrn_i.data ),
      .bit_enable_i   ( '1              ),
      .read_data_o    ( cl_rdata[i]     )
    );

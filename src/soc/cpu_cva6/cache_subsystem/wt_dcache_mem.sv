...
#(
  ...
  for (genvar k = 0; k < DCACHE_NUM_BANKS; k++) begin : gen_data_banks
    ...
    for (genvar j = 0; j < DCACHE_SET_ASSOC; j++) begin: gen_set_assoc
		...
		rf_be_128x128 i_data_rf (
      .clk_i          ( clk_i            ),
      .chip_enable_i  ( bank_req[k]      ),
      .write_enable_i ( bank_we[k]       ),
      .addr_i         ( bank_idx[k]      ),
      .write_data_i   ( bank_wdata[k]    ),
      .bit_enable_i   ( bank_bit_mask[k] ),
      .read_data_o    ( bank_rdata[k]    )
    );
  ...
  for (genvar i = 0; i < DCACHE_SET_ASSOC; i++) begin : gen_tag_srams
    ...
    // Tag RAM
	  ...
	  /* Modifier: Mingxuan Li & Zhantong Zhu */
    rf_be_128x46 i_tag_rf (
      .clk_i          ( clk_i                       ),
      .chip_enable_i  ( vld_req[i]                  ),
      .write_enable_i ( vld_we                      ),
      .addr_i         ( vld_addr                    ),
      .write_data_i   ( {vld_wdata[i], wr_cl_tag_i} ),
      .bit_enable_i   ( '1                          ),
      .read_data_o    ( vld_tag_rdata[i]            )
    );
